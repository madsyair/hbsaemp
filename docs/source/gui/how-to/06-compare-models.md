...
3. Optionally check **Include Bayes Factor (Savage-Dickey, per coefficient)**
   and/or **Include Prior Sensitivity (power-scaling, per parameter)** —
   these are two independent checkboxes, and each costs an extra sampling
   pass per model on top of plain LOO, so both are off by default.
...
5. Read the **Ranking (LOO / ELPD)** table, the ranking/posterior predictive/
   parameter plots, and the Bayes Factor and/or Prior Sensitivity tables if
   enabled.
